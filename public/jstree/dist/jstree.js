	...
	$.jstree.core.prototype = {
      ...
      if (obj && $.inArray(obj.id, this._model.changed) === -1) {
		...
		create_node : function (par, node, pos, callback, is_loaded) {
			...
			if(!node) { node = { "text" : this.get_string('New Doc') }; }
			if(typeof node === "string") {
				node = { "text" : node };
			} else {
				node = $.extend(true, {}, node);
			}
			if(node.text === undefined) { node.text = this.get_string('New Doc'); }
			...
			/*if(newNode){
				$.ajax({
					...
					data:{id:node.id,'pid':par.id, 'text':node.text, 'type':node.type},
					dataType:'json',
 ...
 */
	...
	(function ($) {
		...
		$(function () {
			...
			vakata_context.element = $("<ul class='vakata-context' style='z-index:1000'></ul>");
			vakata_context.element
